...
function addEarthquakeMarker(index) {
    if (index < data.features.length) {
        ...
        document.getElementById("clock-text").textContent = "Earthquake Time: " + dayTime;
...
panToLargestButton.addEventListener("click", function () {
    ...
    if (largestMagnitudeIndex !== -1) {
        ...
        console.log("Time:", new Date(largestEarthquake.properties.time).toLocaleString());
        ...
        earthquakeInfoDiv.innerHTML = `
            ...
            <p><strong>Time:</strong> ${new Date(largestEarthquake.properties.time).toLocaleString()}</p>
        `;
    }
});
